...
(310,32-33): run-time warning JS1014: Invalid character: `
(13,9637-9642): run-time error JS1137: 'class' is a new reserved word and should not be used as an identifier: class
(13,9791-9796): run-time error JS1137: 'class' is a new reserved word and should not be used as an identifier: class
(25,811-818): run-time error JS1137: 'default' is a new reserved word and should not be used as an identifier: default
(25,878-885): run-time error JS1137: 'default' is a new reserved word and should not be used as an identifier: default
 */
...
$(document).ready(function () {
    ...
    /*mobile menu carousel initialization*/
    $('.npa-sidebar-mobile-slick').slick({
        dots: true,
        prevArrow: false,
        nextArrow: false,
        infinite: false,
    ...
    $("body").on('click', '[data-action="open-model-favorite"]', function () {
        $.ajax({
            type: "POST",
            url: "/api/sitecore/PaPostIts/ShowModalFavoriteAdd",
            success: function (data) {
                $("#dropdown-modal").html(data);
    ...
    $("body").on('click', '[data-action="open-model-favorite-remove"]', function () {
        var name = $(this).closest("li").find(".favorite-name").text();
        var title = $(this).closest(".mod-content").data("delete-question");
        var dataAction = $(this).closest(".mod-content").data("confirm-question");

        $.ajax({
            type: "POST",
            url: "/api/sitecore/PaPostIts/ShowFavoriteConfirmModal",
            data: { id: $(this).data("id"), name: name, title: title, action: dataAction },
            success: function (data) {
                $("#dropdown-modal").html(data);
    ...
    $("body").on('click', '[data-action="favorite-removeItem"]', function () {
        var id = $(this).closest(".modal-dialog").data("id");

        $.ajax({
            type: "POST",
            url: "/api/sitecore/PaPostIts/RemoveFavorite",
            data: { id: id },
            success: function (data) {
                if (data.response === 1) {
                    $('#confirmModal').modal("hide");
                    $(".sidebar-favorites").find(".links").find("[data-id='" + id + "']").closest("li").remove();
                }
            },
            error: function (xhr) {
                console.log(xhr);
    ...
    $("body").on('click', '[data-action="save-favorite-item"]', function () {
        var id = $(".sidebar-favorites .title").data("structure-id");
        var title = $("#addFavoriteModal").find("[name='title']").val();
        var link = $("#addFavoriteModal").find("[name='link']").val();

        $.ajax({
            type: "POST",
            url: "/api/sitecore/PaPostIts/AddFavorite",
            data: { id: id, title: title, link: valideUrl(link) },
            success: function (data) {
                if (data.responseTitle !== null && data !== "") {
                    $('#addFavoriteModal').modal("hide");
                    $('.sidebar-favorites').html(data);
                }
            },
            error: function (xhr) {
                console.log(xhr);
...
function paginateNews(page) {
    $.ajax({
        type: "POST",
        url: $('#urlAjax').val(),
        data: { id: $('#content-list').data("item-id"), page: page },
        success: function (data) {
            var html = $(data).filter("#news-list").html();
            $("#news-list").html(html);
        },
        error: function (data) {
            console.log(data);
...
function CheckLogin() {
    $.ajax({
        type: "POST",
        url: "/api/sitecore/Form/ValidateLogin",
...
function RemoveLogin() {
    $.ajax({
        type: "POST",
        url: "/api/sitecore/Form/RemoveLogin",
        success: function (result) {
            console.log(result);
        },
        error: function (result) {
...
function showModalByExternalSource(title, source, height) {
    $.ajax({
        type: "POST",
        url: "/api/sitecore/PaMenu/OptionsModal",
        data: { title: title, source: source, height: height},
        success: function (data) {
            $("#dropdown-modal").html(data);
...
function showModalByAction(url) {
...
function changeProfileImage(url) {

    var $form = $('#profile-image-form');
    var $errorMsg = $form.find('.error-update-photo');

    $errorMsg.hide();
    var formData = new FormData();
    formData.append('profilePhotoFile', $form.find('#profilePhotoFile')[0].files[0]);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            console.log(data);
            if (data && data.Success && data.PortraitURL) {
                $('.avatar-div img').attr('src', data.PortraitURL);
                $("#modalWindow .modal, .modal-backdrop").remove();
            } else {
                $errorMsg.show();
            }
        }
...
function saveToFile(categoria, descricao) {
    var URL = $('#URLAjax').val();
    var URLAllNotifications = $('#URLAllNotifications').val();
    var URLReadNotifications = $('#URLReadNotifications').val();

    $.ajax({
        type: "POST",
        url: URL,
        data: { categoria: categoria, descricao: descricao, urlget: URLAllNotifications, urlread: URLReadNotifications },
        success: function (data) {
            $("#dropdown-modal").html(data);
            $('#modalNotifications').modal('show');
        },
...
function onRead(id, count) {
    var URL = $('#URLReadNotification').val();
    var elem = $("tbody").find(`[data-target='#notification-row-${count}']`);
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.Success === true) {
                $(elem).find(".notification-state").first().text("Lida");
                $(elem).find(".sinistro-estado").first().text("Lida");
...
function onReadNDAG(id, count) {
    var URL = $('#URLReadNotification').val();
    var elem = $("tbody").find(`.notification-number-${count}`);
...
function updateNotifications() {
    var URL = $('#URLNotificationsAjax').val();
    $.ajax({
        type: "POST",
        url: URL,
        success: function (result) {
            var html = $(result).find(".dropdown-notifications").html();
            $(".dropdown-notifications").html(html);

            html = $(result).find(".notification-badge").html();
            $(".notification-badge").html(html);

            var text = $.trim($(".notification-badge").first().text());
            if (text == "0") {
...
function printNotification(that) {
    var divToPrint = $(that).closest(".notification-detail-content").find(".notification-html").first();
...
var app = {
	...
	breakpoint : function() {
		...
		if (app.SIZE.wW <= 1199) {
			screen = 'md';
		} else if (app.SIZE.wW <= 1365) {
			screen = 'lg';
		} else if (app.SIZE.wW <= 1499) {
			screen = 'xlg';
		} else {
			screen = 'xxlg';
		}
	...
	init : function() {
		...
		// MODULES -------------------------------------------
		if (app.exists('.js-header-notifications')) {
			app.header.init();
		}
	}
};
...
app.dsi = {
	...
	// ENDPOINT CALL -------------------------------------
	endpointCall : function(getURI, postInfo) {
		return $.post({
			url : getURI,
			data : postInfo,
			dataType : "json"
	...
	showMessage : function(title, text) {
		...
		app.components.modal.init(obj);
	},

	modalIframe : function() {

		var link = $(this).attr('href'),
		heightIncrement = $(this).attr('data-height-increment'),
		selectorHeightCalculator = $(this).attr('data-selector-height-calculator'),
		ctx = $(this).attr('data-ctx');

		var obj = {
			type : 'iframe',
			ctx : ctx,
			heightIncrement : heightIncrement,
			selectorHeightCalculator : selectorHeightCalculator,
			link : link,
			btnCancel : 'Cancelar'
		};
		app.components.modal.init(obj);
...
app.header = {
	...
	// MAIN FUNCTIONS ------------------------------------
	init : function() {
		var _this = app.header;

		_this.notifications.init();
	},
	...
	notifications : {
		elements : {
			getHeaderMenu : function() {
				return $('.notificationHeaderBtn');
			},
			getCategoryLst : function() {
				return $('.notifications .notifications-wrapper .notification-categories');
			},
			getNotificationsBadge : function() {
				return $('.notifications .notificationsButton .notifications-badge');
			}
		},

		click : function(evt) {
			if (!evt.data.$header.hasClass('notifications-is-opened')) {
				evt.data._notifications.open(evt.data);
			} else {
				evt.data._notifications.close(evt.data);
			}
		},

		open : function(edata) {
			edata.$header.addClass('notifications-is-opened');
			edata.$btnNotifications.addClass('is-active');
		},
		close : function(edata) {
			edata.$header.removeClass('notifications-is-opened');
			edata.$btnNotifications.removeClass('is-active');
		},
		init : function() {
			_this = app.header.notifications;
			_this.processIntrusiveNotifications(USR_PIORITY_MESSAGES);
		},
		readCategories: function () {
			$.ajax({
				type: "POST",
				url: '/api/sitecore/Notifications/GetNotificationsAjax',
				success: function (result) {
					var html = $(result).find(".dropdown-notifications").html();
					$(".dropdown-notifications").html(html);
		...
		readMessage: function (notificationId, successCb) {
			if (notificationId) {
				$.when(app.dsi.endpointCall('/api/sitecore/Notifications/ReadNotification', { id: notificationId })).done(function (response) {
					if (response && response.Success === true) {
						app.header.notifications.readCategories();
		...
		processIntrusiveNotifications : function(intrusiveNotifications) {
			if (intrusiveNotifications && intrusiveNotifications.length > 0) {
				var _this = app.header.notifications;
				
				var notification = intrusiveNotifications[0];

				app.components.modal.init({
					type : "intrusiveNotification",
					title : notification.Assunto,
					html : notification.Texto,
					onClose: function () {
						intrusiveNotifications.shift();
						if (intrusiveNotifications.length > 0) {
							_this.processIntrusiveNotifications(intrusiveNotifications);
						}
					}
				});

				_this.readMessage(notification.Id);
			}
		}
	}
};
...
app.sidebar = {
	...
	setupEvents : function() {
		var _this = app.sidebar;
		if (!app.sidebar.EVENTS) {
			app.sidebar.EVENTS = true;

			$doc

			.on('click', '[data-action="favorite-removeItem"]', _this.favorite.remove.ask).on('click', '[data-action="favorite-addItem"]', _this.favorite.add);

			app.sidebar.profile.photo.setup();
		}
	}
};

/*
 * INFO: COMPONENTS - Definição dos componentes globais e genéricos; - Estes
 * componentes poderão ser utilizados em qualquer módulo, página ou ainda por
 * outros componentes.
 */
app.components = {
	...
	getTemplate: function (id) {
		if (id !== '' && id !== undefined) {
			return $($body.find('template[data-id="' + id + '"]').html());
		}
	},

	modal: {
		init: function (obj) {
			var _this = app.components.modal;

			if (obj.type === 'confirm') {
				$.when(_this.confirmHtml(obj)).done(function (html) {

					$("#modalWindow").html(html);
					$("#confirmModal").modal();

					$doc.off('click.confirmCallback').on('click.confirmCallback', '[data-action="confirmCallback"]', obj.onConfirm);

				});
			}

			else if (obj.type === 'intrusiveNotification') {
				$.when(_this.intrusiveNotificationHtml(obj)).done(function (html) {
					$("#modalWindow").html(html);
					...
					$modal.modal();
					if (obj.onClose) {
						$modal.on('hide.bs.modal', obj.onClose);
					}
		...
		confirmHtml: function (obj) {
			var $html = app.components.getTemplate("tpl-modal-confirm");
			$html.find("h6").text(obj.title);
			$html.find(".modal-body .element-name").text(obj.text);
			$html.find(".actions").find("button.btn-cancel").text(obj.btnCancel);
			$html.find(".actions").find("button.btn-confirm").text(obj.btnConfirm);
			return $html[0];
		},

		intrusiveNotificationHtml: function (obj) {
			var $html = app.components.getTemplate('tpl-intrusive-message-modal');

			$html.find('#intrusiveMessageLabel').text(obj.title);
			$html.find('.modal-body').html($("<div/>").html(obj.html).text());
	...
	modal2: {
		init: function (obj) {
			var _this = app.components.modal2;

			if (obj.type === 'confirm') {
				$.when(_this.confirmHtml(obj))
					.done(function (html) {
...

							{
								...
								afterShow: function (instance) {
									$doc
										...
										.on('click.closeModal', '[data-action="closeModal"]', function () {
											if (obj.onCancel) {
												obj.onCancel();
											}
											instance.close();
										})
										.off('click.confirmCallback')
										.on('click.confirmCallback', '[data-action="confirmCallback"]', function () {
											instance.close();
											obj.onConfirm();
										});
								}
				...
				$.when(_this.notificationHtml(obj))
					.done(function (html) {
...

							{
								...
								afterShow: function (instance) {
									$doc
										...
										.on('click.confirmCallback', '[data-action="confirmCallback"]', function () {
											var fields = {};
											$(".modal-wrapper form input").each(function (index, element) {
												fields["" + $(element).attr("id")] = $(element).val();
											}
											);
											instance.close();
											obj.onConfirm(fields);
										});
								}
							});
					});
				...
				$.when(_this.intrusiveNotificationHtml(obj))
					.done(function (html) {
		...
		notificationHtml: function (obj) {
			var $html = app.components.getTemplate('tpl-modal-notification');

			$html.find('h6').text(obj.title);
			$html.find('.content').html(obj.html);
			$html.find('.actions').find('button.btn-cancel').text(obj.btnCancel);
			$html.find('.actions').find('button.btn-confirm').text(obj.btnConfirm);
		...
		intrusiveNotificationHtml: function (obj) {
			var $html = app.components.getTemplate('tpl-modal-intrusive-notification');
